...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("\nMax length:");
        System.out.println(col.stream().max(Comparator.comparing(String::length)).get());
        System.out.println("\nMin length:");
        System.out.println(col.stream().min(Comparator.comparing(String::length)).get());
        System.out.println("\nWords without punctuation marks and spaces:");
        List<String> onlyWords = col.stream().filter(word->word.matches("[a-zA-Z1-9]*")).toList();
